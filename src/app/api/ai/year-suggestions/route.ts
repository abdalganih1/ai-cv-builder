...
`;
            ...
            const major = education[currentEntryIndex || 0]?.major;
            analysisContext = `
المطلوب: اقتراح سنة انتهاء الدراسة لشهادة "${degree || 'غير محدد'}"${major ? ` في تخصص "${major}"` : ''}

سنة البدء: ${startYear || 'غير محددة'}

مدة الدراسة المتوقعة (سوريا):
- بكالوريوس هندسة (كل التخصصات الهندسية بما فيها الاتصالات/المعلوماتية/الحواسيب/التحكم الآلي/المعمار): 5 سنوات إلزامياً
- بكالوريوس عادي: 4 سنوات
- ماجستير: سنتين
- دكتوراه: 3 سنوات
- دبلوم: سنتين
...
2. إذا التخصص هندسي والشهادة بكالوريوس: المدة 5 سنوات دائماً
3. أضف خيار "حالياً (لا أزال طالباً)" دائماً
4. أرجع JSON array: [{"year": 2029, "label": "2029 (5 سنوات - هندسة)"}, ...]
`;
        }
